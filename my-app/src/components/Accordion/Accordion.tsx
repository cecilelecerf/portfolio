...
import { Accordion, AccordionControl, AccordionItem, AccordionPanel, Center, FloatingIndicator, Image, SimpleGrid, Stack, Tabs, TabsTab, Text } from "@mantine/core";
                    ...
                    <Tabs variant="none" value={value} onChange={setValue} >
                        <Tabs.List ref={setRootRef} className={classes.list}>
                            {Object.entries(allOutils).map(([key, outil], i) =>
                                <TabsTab value={key} key={i} ref={setControlRef(i.toString())} onClick={() => setValue(i.toString())} className={classes.tab}>{outil.label}</TabsTab>
                            )}
                            <FloatingIndicator
                        ...
                        {Object.entries(allOutils).map(([key, outil], i) =>
                            <Tabs.Panel value={i.toString()} key={i}>
                                <SimpleGrid cols={3}>
                                    {Object.entries(outil.infos).map(([key, value], i) => <Item key={i} item={value} />)}
                                </SimpleGrid>
                            </Tabs.Panel>
                        )}
                    </Tabs>
